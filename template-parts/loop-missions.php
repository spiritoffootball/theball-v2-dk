<?php
/**
 * Template part for embedding a display of all Missions.
 *
 * These are, in fact, pages with the "To missioner" page as parent.
 *
...
// Get parent page.
$loop_parent = get_page_by_path( 'mission', OBJECT, 'page' );
...
if ( $loop_parent instanceof WP_Post ) :
	...
	if ( $loop_include->have_posts() ) : ?>
		...
		<!-- loop-missions.php -->
		<section id="missions" class="loop-include loop-include-two content-area clear">
			<div class="loop-include-inner">

				<header class="loop-include-header">
					<h3 class="loop-include-title"><?php esc_html_e( 'What we do', 'theball-v2-dk' ); ?></h3>
					...
					<?php if ( ! empty( $page_excerpt ) ) : ?>
						<div class="mission-page-excerpt">
				...
				<div class="loop-include-posts">
					...
					while ( $loop_include->have_posts() ) :
						...
						// Get mini template.
						get_template_part( 'template-parts/content-mission-mini' );
