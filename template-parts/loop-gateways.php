<?php
/**
 * Template part for embedding a display of all Gateway Items.
 *
 * These are, in fact, pages with the "Fundraising & partnerskaber" page as parent.
 *
...
// Get parent page.
$loop_parent = get_page_by_path( 'gateway', OBJECT, 'page' );
...
if ( $loop_parent instanceof WP_Post ) :
	...
	if ( $loop_include->have_posts() ) : ?>
		...
		<!-- loop-gateways.php -->
		<section id="gateways" class="loop-include loop-include-three content-area clear">
			<div class="loop-include-inner">

				<header class="loop-include-header">
					<h3 class="loop-include-title"><?php esc_html_e( 'Nordic arm', 'theball-v2-dk' ); ?></h3>
				...
				<div class="loop-include-posts">
					...
					while ( $loop_include->have_posts() ) :
						...
						// Get mini template.
						get_template_part( 'template-parts/content-gateway-mini' );
