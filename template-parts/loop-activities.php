<?php
/**
 * Template part for embedding a display of all Activities.
 *
 * These are, in fact, pages with the "Hvad vi gør i praksis" page as parent.
 *
...
// Get parent page.
$loop_parent = get_page_by_path( 'praksis', OBJECT, 'page' );
...
if ( $loop_parent instanceof WP_Post ) :
	...
	if ( $loop_include->have_posts() ) : ?>
		...
		<!-- loop-activities.php -->
		<section id="activities" class="loop-include loop-include-three content-area clear">
			<div class="loop-include-inner">

				<header class="loop-include-header">
					<h3 class="loop-include-title"><?php esc_html_e( 'Activities', 'theball-v2-dk' ); ?></h3>
					...
					<?php if ( ! empty( $page_excerpt ) ) : ?>
						<div class="activity-page-excerpt">
				...
				<div class="loop-include-posts">
					...
					while ( $loop_include->have_posts() ) :
						...
						// Get mini template.
						get_template_part( 'template-parts/content-activity-mini' );
