<?php
/**
 * Template part for embedding a display of all SOF Organisations.
 *
 * These are, in fact, pages with the "En global familie" page as parent.
 *
...
// Get parent page.
$loop_parent = get_page_by_path( 'global-familie', OBJECT, 'page' );
...
if ( $loop_parent instanceof WP_Post ) :
	...
	if ( $loop_include->have_posts() ) : ?>
		...
		<!-- loop-sof-orgs.php -->
		<section id="sof-orgs" class="loop-include loop-include-four content-area clear">
			<div class="loop-include-inner">

				<header class="loop-include-header">
					<h3 class="loop-include-title"><?php esc_html_e( 'The Spirit of Football family', 'theball-v2-dk' ); ?></h3>
				...
				<div class="loop-include-posts">
					...
					while ( $loop_include->have_posts() ) :
						...
						// Get mini template.
						get_template_part( 'template-parts/content-sof-org-mini' );
